...
public class KeyServlet extends HttpServlet{ 
	...
	private void saveKey(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
		String inputKey = new String(request.getParameter("suggest").getBytes("ISO-8859-1"), "UTF-8");
		ConnDB conn = new ConnDB();
		if ((inputKey != null) && (!"".equals(inputKey))) {
			String sql = "SELECT * FROM tb_key WHERE keyWord ='%" + inputKey + "%'";
			ResultSet rs = conn.executeQuery(sql);
			try{
				if (!rs.next()) {
					String sqlIns = "INSERT INTO tb_key (keyWord) VALUES('" + inputKey + "')";
					int i = conn.executeUpdate(sqlIns);
				}
			}
			catch (SQLException e) {
				e.printStackTrace();
